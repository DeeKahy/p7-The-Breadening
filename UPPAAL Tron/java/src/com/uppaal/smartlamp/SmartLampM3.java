...
public class SmartLampM3 extends VirtualThread implements LampInterface
{
	...
	while (started) {
	    System.out.println("IFace state: "+location);
	    switch (location) {
	    case Idle:
		cond.await();
		break;
	    case Ignoring:
		cond.await(epsilon, TimeUnit.MILLISECONDS);
		if (location == Loc.Ignoring) // must have been timeout
		    location = Loc.Alert;
		break;
	    case Alert:
		cond.await(delta-epsilon, TimeUnit.MILLISECONDS);
		if (location == Loc.Alert) { // must have been timeout
	...
	System.err.println("IFace interrupted in "+location);
	dimmer.interrupt();
	reporter.disconnect();
	lock.unlock();
    }

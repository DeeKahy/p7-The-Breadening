...
echo -e "${GREEN}=== Starting Mutex Testing Environment (WebSocket) ===${NC}"
echo "Logs will be saved in ./logs/"
echo ""

# Check for required Python packages
echo -e "${BLUE}[0/4] Checking Python dependencies...${NC}"
...
echo -e "${BLUE}[1/4] Checking Java dependencies...${NC}"
mkdir -p lib
...
# Compile the adapter
echo -e "${BLUE}[2/4] Compiling MutexAdapter...${NC}"
javac -cp ".:src:lib/Java-WebSocket-1.5.3.jar:lib/json-20230227.jar:lib/slf4j-api-2.0.7.jar" -d build src/com/uppaal/tron/Adapter.java src/com/uppaal/tron/Reporter.java src/mutex/MutexAdapter.java
if [ $? -ne 0 ]; then
...
echo -e "${BLUE}[3/4] Starting Flask WebSocket server (port 5000)...${NC}"
cd dut
...
echo -e "${GREEN}✓ Flask WebSocket server running${NC}"
echo ""

# 2. Start Java adapter
echo -e "${BLUE}[4/4] Starting MutexAdapter (port 9999)...${NC}"
